...
def _get_input_node_keys(n_input):
    return list(range(-1, -n_input - 1, -1))


def _get_output_node_keys(n_output):
...
class LayerBuilder:
    ...
    def create(self):
        ...
        for connection_key, connection in self.connections.items():
            if isinstance(connection_key, str):
                connection_key_corrected = Genome._get_connection_key_from_key_str(connection_key)
            else:
            ...
            if output_node_key in layer_node_keys:
                ...
                layer_connections[connection_key_corrected] = self.connections[connection_key]
    ...
    def _build_weight_tensors(layer_connections, input_node_keys, layer_node_keys, n_input, n_output):
        ...
        for connection_key in layer_connections:
            ...
            weight_mean[key_index_mapping_output[output_node_key], key_index_mapping_input[input_node_key]] = \
                float(layer_connections[connection_key]['mean_weight'])
            weight_log_var[key_index_mapping_output[output_node_key], key_index_mapping_input[input_node_key]] = \
                float(_log_var(layer_connections[connection_key]['std_weight']))
    ...
    def _build_bias_tensors(layer_node_keys, nodes):
        ...
        '''
        bias_mean_values = [float(nodes[str(key)]['mean_bias']) for key in layer_node_keys]
        bias_mean = torch.tensor(bias_mean_values)

        bias_log_var_values = [float(_log_var(std=nodes[str(key)]['std_bias'])) for key in layer_node_keys]
...
def _log_var(std):
    return np.log(np.square(std)) - 1.0
